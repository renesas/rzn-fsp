...
fsp_err_t R_USB_HHID_TypeGet (usb_ctrl_t * const p_ctrl, uint8_t * p_type, uint8_t device_address)
{
    ...
    FSP_ERROR_RETURN(!(USB_IP1 == p_instance_ctrl->module), FSP_ERR_USB_PARAMETER);
 #endif                                /* defined(BSP_MCU_GROUP_RA2A1) */
#endif /* USB_CFG_PARAM_CHECKING_ENABLE == BSP_CFG_PARAM_CHECKING_ENABLE */

    err = R_USB_InfoGet(p_instance_ctrl, &info, device_address);
    FSP_ERROR_RETURN(!((FSP_SUCCESS != err) || (USB_STATUS_CONFIGURED != info.device_status)), FSP_ERR_USB_FAILED);
    *p_type = usb_hhid_get_hid_protocol(p_instance_ctrl->module_number, device_address);
...

{
 ...
 #if defined(BSP_MCU_GROUP_RA2A1)
    FSP_ASSERT(USB_IP1 != p_instance_ctrl->module_number);
 #endif                                /* defined(BSP_MCU_GROUP_RA2A1) */
    ...
    {
        pipe = usb_hstd_get_pipe_no(p_instance_ctrl->module_number,
                                    device_address,
    ...
    err = R_USB_UsedPipesGet(p_instance_ctrl, &pipe_bit_map, device_address);
    ...
    utr.keyword = p_instance_ctrl->device_address;
