...
/* Worst case ratio of (CPUnCLK/PCLKSPIn) = 5.333... -> 6 bytes approximately.
 * A synchronization delay time of 3PCLKSPIn is required for signal propagation.
 */
#define SPI_PERIPHERAL_REG_MAX_WAIT     (0x06U * 0x03U)
...
static void r_spi_hw_config (spi_instance_ctrl_t * p_instance_ctrl)
{
    ...
    spcmd |= (uint32_t) p_instance_ctrl->p_cfg->clk_phase << R_SPI0_SPCMD_CPHA_Pos;

    /* Configure CPOL setting. */
    spcmd |= (uint32_t) p_instance_ctrl->p_cfg->clk_polarity << R_SPI0_SPCMD_CPOL_Pos;

    /* Configure Bit Order (MSB,LSB) */
    spcmd |= (uint32_t) p_instance_ctrl->p_cfg->bit_order << R_SPI0_SPCMD_LSBF_Pos;
    ...
    if (SPI_SSL_MODE_SPI == p_extend->spi_clksyn)
    {
        ...
        spcmd |= (uint32_t) (!p_instance_ctrl->p_cfg->operating_mode << R_SPI0_SPCMD_SSLKP_Pos);
#endif
    ...
    p_instance_ctrl->p_regs->SPCMD[0] = spcmd;
